...
function CommunityFormPreview({ onNavigate }: GetAppPageProps) {
  return (
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 shadow-xl shadow-navy/5">
      <h3 className="text-2xl sm:text-3xl font-display text-foreground">Join the Classy Community</h3>
      ...
      <div className="mt-6 max-h-[min(440px,70vh)] overflow-y-auto rounded-lg border border-border bg-[#f1eefc] p-4">
        <div className="overflow-hidden rounded-lg bg-card shadow-sm">
          <div className="h-3 bg-[#673ab7]" />
          <div className="border-b border-border p-5">
            <h4 className="text-xl sm:text-2xl font-body font-semibold text-foreground">Users - Classy</h4>
            <p className="mt-4 text-sm font-body font-semibold text-mid-grey">
              Secure preview account <span className="font-normal text-blue underline">Switch account</span>
            </p>
      ...
      <p className="mt-4 text-xs font-body text-mid-grey">
        By submitting this form you agree to our <button type="button" onClick={() => onNavigate("privacy")} className="text-blue underline-offset-2 hover:underline">Privacy Policy</button> and{" "}
        <button type="button" onClick={() => onNavigate("terms")} className="text-blue underline-offset-2 hover:underline">Terms &amp; Conditions</button>.
      </p>
    </div>
  );
}
...
type RoleFeature = (typeof roleFeatures)[number] & { Icon: LucideIcon };

function RolePreview({ role }: { role: RoleFeature }) {
  return (
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 shadow-xl shadow-navy/5">
      <div className="rounded-lg border border-border p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {role.stats.map((stat, index) => (
            <div
              key={stat}
              className={`rounded-lg p-4 text-sm font-body min-h-14 ${
...
export function GetAppPage({ onNavigate }: GetAppPageProps) {
  ...
  const activeRoleFeature = roleFeatures[activeRole] as RoleFeature;
      ...
      <section className="section-pad bg-card">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="rev flex flex-wrap justify-center gap-3 border-b border-border pb-6 mb-10">
            {roleFeatures.map((role, index) => (
              <button
                ...
                className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-body font-semibold transition-all ${
                  activeRole === index
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="rev">
              <h3 className="text-3xl md:text-4xl font-display mb-4">{activeRoleFeature.title}</h3>
              <p className="text-mid-grey font-body leading-relaxed mb-8 max-w-xl">{activeRoleFeature.desc}</p>
              <div className="flex flex-wrap gap-3">
                {activeRoleFeature.chips.map((chip) => (
            ...
            <div className="rev" style={{ transitionDelay: "120ms" }}>
              <RolePreview role={activeRoleFeature} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section ref={downloadRef} className="section-pad bg-card">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div className="min-w-0 space-y-6 rev">
              {/* Android */}
              <div className="min-w-0 overflow-hidden rounded-xl border-2 border-blue/30 bg-card p-5 shadow-lg shadow-blue/5 sm:p-6">
                <div className="flex min-w-0 items-center gap-3 mb-3">
                  <Smartphone className="h-8 w-8 shrink-0 text-blue" />
                  <div className="min-w-0">
                    <p className="font-body font-bold text-foreground">Android App</p>
                    <p className="break-words text-sm text-mid-grey font-body">Compatible with Android 8.0 and above</p>
                  </div>
                </div>
                <a
                  href="https://play.google.com/store/apps/details?id=com.classy.mobile&pli=1"
                  target="_blank"
                  rel="noopener noreferrer"
                  referrerPolicy="no-referrer"
                  aria-label="Download Classy on Google Play Store"
                  className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-blue px-4 py-3 text-sm font-body font-semibold text-primary-foreground shadow-md shadow-blue/20 transition-all hover:brightness-110 sm:px-6 mb-2"
                >
                  <Download className="h-4 w-4 shrink-0" /> <span className="min-w-0 text-center">Download on Play Store</span>
                </a>
                <p className="text-xs text-mid-grey font-body text-center">Available on Google Play · Free download</p>
              </div>

              {/* iOS */}
              <div className="min-w-0 overflow-hidden rounded-xl border border-border bg-muted/30 p-5 sm:p-6">
                <div className="flex min-w-0 items-center gap-3 mb-3">
                  <Apple className="h-8 w-8 shrink-0 text-mid-grey" />
                  <div className="min-w-0">
                    <p className="font-body font-bold text-foreground">iPhone &amp; iPad</p>
                    <p className="break-words text-sm text-mid-grey font-body">iOS 14 and above</p>
                  </div>
                </div>
                <button type="button" disabled className="w-full border-2 border-border text-mid-grey rounded-lg px-6 py-3 text-sm font-body font-semibold opacity-60 cursor-not-allowed mb-2">
                  Coming Soon
                </button>
              ...
              <div className="min-w-0 overflow-hidden rounded-xl border border-border bg-muted/20 p-5">
                <p className="break-words text-sm font-body text-mid-grey mb-4">Listing in progress. Classy is currently being reviewed for listing on the Google Play Store and Apple App Store. Once approved, you'll be able to install directly without any extra steps.</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {["Google Play", "App Store"].map((store) => (
                    <div key={store} className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-center">
                      <span className="block text-xs font-body font-semibold text-foreground">{store}</span>
                      <span className="mt-1 block text-xs font-body text-mid-grey">Coming Soon</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="max-w-4xl mx-auto text-center rev">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-primary-foreground/10 backdrop-blur-sm rounded-xl p-8 border border-primary-foreground/20">
              <School className="mx-auto mb-4 h-9 w-9 text-primary-foreground" aria-hidden="true" />
              <p className="font-body font-bold text-primary-foreground text-lg mb-4">I represent an educational institute</p>
              <button type="button" onClick={() => onNavigate("pricing")} className="w-full bg-primary-foreground text-navy rounded-lg px-6 py-3 text-sm font-body font-semibold hover:brightness-95 transition-all">View Pricing →</button>
            </div>
            <div className="bg-primary-foreground/10 backdrop-blur-sm rounded-xl p-8 border border-primary-foreground/20">
              <UsersRound className="mx-auto mb-4 h-9 w-9 text-primary-foreground" aria-hidden="true" />
              <p className="font-body font-bold text-primary-foreground text-lg mb-4">I'm a parent or teacher</p>
              <button type="button" onClick={scrollToDownload} className="w-full border-2 border-primary-foreground text-primary-foreground rounded-lg px-6 py-3 text-sm font-body font-semibold hover:bg-primary-foreground/10 transition-all">Download the App →</button>
